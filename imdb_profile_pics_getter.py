...
def get_actor_pics_by_id(actor_id, dl_folder, headers={'user-agent': 'my-app/0.0.1'}, disp=True, save_to_device=True):
    ...
    for page_no in range(1, max_page_count+1):
        ...
        for index, img_link in enumerate(img_links):
            ...
            alt_link = soup.findAll('img')[1]['alt']
            if disp: print(f"  {index+1} / {img_links_count}: {src_link}")
            final_src_links.append(src_link)

            if save_to_device:
                if save_image(src_link, alt_link, dl_folder):
                    print("f Could not save: {alt_link}: [{src_link}]")
                    return 1

    return list(set(final_src_links))
