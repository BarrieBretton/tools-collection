...
def dupe_actions(dupes, action="display", discard_protocol="r"):
    ...
    if action in actions:
        ...
        if action == 0:
            ...
            for src, matches in dupes.items():
                ...
                if older_mtime_matches: discard_pile.extend(older_mtime_matches)
                else: discard_pile.append(src)

        elif action == 3:
            for src, matches in dupes.items():
                older_ctime_matches = [match for match in matches if os.path.getctime(match) < os.path.getctime(src)]
                if older_ctime_matches: discard_pile.extend(older_ctime_matches)
                else: discard_pile.append(src)

        elif action == 4:
            for src, matches in dupes.items():
                newer_mtime_matches = [match for match in matches if os.path.getmtime(match) > os.path.getmtime(src)]
                if newer_mtime_matches:
                    newest_mtime_match = max(newer_mtime_matches)
                    discarded_matches = [i for i in newer_mtime_matches if i != newest_mtime_match]
                    discard_pile.extend(discarded_matches)
                else:
                    discard_pile.append(src)

        elif action == 5:
            for src, matches in dupes.items():
                newer_ctime_matches = [match for match in matches if os.path.getctime(match) > os.path.getctime(src)]
                if newer_ctime_matches:
                    newest_ctime_match = max(newer_ctime_matches)
                    discarded_matches = [i for i in newer_ctime_matches if i != newest_ctime_match]
                    discard_pile.extend(discarded_matches)
                else:
                    discard_pile.append(src)
    ...
    if discard_protocol == "d":
        ...
        print("Do you want to confirm their isolation to a separate directory")
        iso_dir = input("Please enter dir path for isolation: ")
        src_name = os.path.basename(src)

        for i, path in enumerate(discard_pile):
            dest_path = os.path.join(iso_dir, f"[{i+1}]_{src_name}")
            # shutil.move(os.path.join(src_path, dest_path))
...
def main():
    ...
    if argc in [1, 2] and sys.argv[1] != '-s':
