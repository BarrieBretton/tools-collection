...
reqdmodules = 'webbrowser;clipboard;psutil;subprocess;pickle;datetime;time;calendar;google;holidays;json;shlex'.split(';')
...
# except:
#     try:
#         subprocess.Popen('pip install clipboard')
#     except:
#         print('err\\\\WarnClipboardload -- err\\WarnProg \n~$ Sorry, can\'t copy and paste pwds to and from the clipboard...')

def wifienable():
    subprocess.Popen("netsh interface set interface 'WiFi' enabled")

def wifidisable():
    subprocess.Popen("netsh interface set interface 'WiFi' disabled")

def showwifinets():
    subprocess.Popen('"netsh wlan show profiles"')
...
def start():
    ...
    def forceshellterminate():
        ...
        for i in PROCNAME:
            ...
            print(i in (p.name() for p in psutil.process_iter()))
                ...
                #     # subprocess.Popen('TASKKILL /F /IM '+i)
                # except:
                #     pass
            ...
            #     subprocess.Popen(shlex.split('TASKKILL /F /IM '+PROCNAME), shell=True)
    ...
    if (psutil.sensors_battery().percent <= lowbper and not psutil.sensors_battery().power_plugged == True) and powerdisclaimerdisplayedval == 0:
        ...
        if lbrightnesspermval == 'y':
            subprocess.Popen(shlex.split('powershell -c (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1,30)'), shell=True)
        else:
            pass
    ...
    def check():
        ...
        if (bp >= 95 and plugged == 1 and Ival == 'nI'):
            subprocess.Popen(shlex.split('mshta \"javascript:var sh=new ActiveXObject( \'WScript.Shell\' ); sh.Popup( \'Battery almost full!, consider removing charger...\', 10, \'Battery Warning\', 64 );close()\"'), shell=True)
        ...
        if c == 'exit':
            ...
            subprocess.Popen('start cmd.exe'.split(), shell=True)

        elif c == 'stop':
            stopval = 1

        elif c == 'teams':
            subprocess.Popen('''"C:\\Users\\Vivo Jay\\AppData\\Local\\Microsoft\\Teams\\Update.exe" --processStart "Teams.exe"''', shell=True)
            ...
            subprocess.Popen(shlex.split('taskkill /f /im firefox.exe'), shell=True)
            ...
            dirs = [_dir for _dir in os.listdir() if _dir.startswith('Mariana Music Player')]
            setup_pypaths_jsons = []
            locally_available_branches = []
            latest_locally_available_branch = None

            for _dir in dirs:
                print(os.path.join(os.environ['localappdata'], _dir ,'setup.pypaths'))
                if os.path.isfile(os.path.join(_dir ,'setup.pypaths')):
                    try:
                        with open(os.path.join(_dir ,'setup.pypaths'), 'r', encoding='utf-8') as fp:
                            setup_pypaths_json = json.load(fp)
                            if set(setup_pypaths_json.keys()) == {"INSTALLED_PYS", "PROG_DIR_PATH", "SETUP_COMPLETE", "SUPPORTED_PY_VERSIONS", "THIS_BRANCH", "BRANCHES"}:
                                locally_available_branches.append(setup_pypaths_json['THIS_BRANCH'])
                                setup_pypaths_jsons.append(setup_pypaths_json)
                    except Exception:
                        raise

            if setup_pypaths_jsons == []:
                print("Sorry, but you don't seem to have Mariana Player installed, download from here: https://www.dropbox.com/s/jtw33u265anaeyo/INSTALLATION.py?dl=1")

            else:
                for setup_pypaths_json in setup_pypaths_jsons:
                    if setup_pypaths_json:
                        PROG_DIR_PATH = setup_pypaths_json['PROG_DIR_PATH']
                        SETUP_COMPLETE = setup_pypaths_json['SETUP_COMPLETE']
                        BRANCHES = setup_pypaths_json['BRANCHES']

                        if SETUP_COMPLETE:
                            if os.path.isdir(PROG_DIR_PATH):
                                locally_available_branches_enumerated = []
                                for index, branch in enumerate(BRANCHES):
                                    if branch in locally_available_branches:
                                        locally_available_branches_enumerated.append((index, branch))

                                locally_available_branches_enumerated.sort(key=lambda x: int(x[0]))
                                latest_locally_available_branch = locally_available_branches_enumerated[0]


                if latest_locally_available_branch:

                    setup_pypaths_json = setup_pypaths_jsons[latest_locally_available_branch[0]]
                    PROG_DIR_PATH = setup_pypaths_json['PROG_DIR_PATH']


                music_player_runner_path = os.path.join(PROG_DIR_PATH, 'mariana.bat')

                # try: subprocess.run(['start', "", music_player_runner_path], shell=True)
                # except Exception: print("ERROR: Cannot open Mariana Player due to unknown reason...")

                try: subprocess.run(music_player_runner_path, shell=True)
                except Exception: print("ERROR: Cannot open Mariana Player due to unknown reason...")

        elif c == 'setup':
            # Open Teams
            openproglist = ''' ""C:\\Program Files (x86)\\Google\\Chrome Beta\\Application\\chrome_proxy.exe" --profile-directory=Default --app-id=hmjkmjkepdijhoojdojkdfohbdgmmhki"; ""C:\\Users\\Vivo Jay\\AppData\\Local\\Microsoft\\Teams\\Update.exe" --processStart "Teams.exe""; "C:\Program Files (x86)\BraveSoftware\Brave-Browser\Application\brave.exe"; "" '''.strip().replace('; ', ';').split(';')
            ...
            for b in openproglist:
                try:
                    subprocess.Popen(shlex.split(b), shell=True)
                except:
                    print('error', b)
            try:
                ...
                print('\nwarn\\\\NonChromeBrowserUnavailableinVer\n~$ This version does not support browsers other than chrome, edge and firefox at the moment;\
                 \n[Feature might be available in one of the upcoming version(s) ;-D...]\n')

            subprocess.Popen(shlex.split('start "" "C:/Users/Vivo Jay/Desktop/tt.png"'), shell=True)


        elif c in ['explorer', 'xyplorer']:
            subprocess.Popen('"C:\Program Files (x86)\XYplorerFree\XYplorerFree.exe"')

        elif c == 'explorer-def':
            subprocess.Popen('explorer', shell=True)

        elif c == 'open zoom':
            p = input("Really want to open \"ZOOM\" (y/n)? ").lower().strip()
            if p == 'y':
                try:
                    subprocess.Popen('"start zoom"')
                except:
                    subprocess.Popen('"C:\\Users\\Vivo Jay\\AppData\\Roaming\\Zoom\\bin"')
            else:
                print("Cancelling opening zoom")
            ...
            if perm == 'yes':
                print('Restarting...')
                subprocess.Popen(shlex.split('shutdown -r -t 00'), shell=True)

        elif c == '//sht':
            perm = input('You have invoked the restart command\ndo you wish to restart immediately?\n ["yes"/"no"] (Type the exact word): ')
            if perm == 'yes':
                print('Shutting down...')
                subprocess.Popen(shlex.split('shutdown -s -t 00'), shell=True)
            ...
            try:
                ...
                try:
                    subprocess.Popen('pip install ctypes')
                    user32 = ctypes.windll.user32
                    screensize = ('Width:', str(user32.GetSystemMetrics(0)), 'Height:', str(user32.GetSystemMetrics(1)))
                    print(screensize)
            ...
            subprocess.Popen('start powerpnt.exe'.split(), shell=True)

        elif c == 'word':
            subprocess.Popen('start winword.exe'.split(), shell=True)

        elif c == 'excel':
            subprocess.Popen('start excel.exe'.split(), shell=True)

        elif c == 'onenote':
            subprocess.Popen('start onenote'.split(), shell=True)

        elif c == 'camera' or c == 'cam':
            campermval = input('Do you really want to start the camera?\n[Type \'y\' for yes or \'n\' for no]')
            if campermval == 'y':
                subprocess.Popen('start microsoft.windows.camera:'.split(), shell=True)
            else:
            ...
            subprocess.Popen(r'"%userprofile%\AppData\Local\Microsoft\WindowsApps\Skype.exe"')
            ...
            subprocess.Popen('wmic process where name="taskmgr.exe" call terminate')
            ...
            if c.isnumeric() == True:
                c = int(c)
                subprocess.Popen('powershell -c (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1, ' + str(c) + ')')

            elif c in ['medium', 'med']:
                subprocess.Popen('powershell -c (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1, 60)')
                ...
                subprocess.Popen('powershell -c (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1, 85)')

            elif c in ['full', 'max']:
                subprocess.Popen('powershell -c (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1, 100)')
            ...
            subprocess.Popen('taskkill /f /im cortana.exe')
...
subprocess.Popen(['start', "", "D:\_Programs\AutoDarkMode\AutoDarkModeApp.exe"], shell=True)
start()
